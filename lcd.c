...
*****************************************************************************/
{
  return( xQueueSend( lcd_queue, &Ch, 10 ));
}
...
{
    INT8U ch;
    static INT8U my_state = LCD_POWER_UP;
    ...
    while (1) // all tasks must be in an infinite while loop as they cannot return or RTOS breaks
    {
        ...
        case LCD_READY:
            ...
            // uncomment if you want to test if lcd is working
            /*
            else{
                INT8U c = 0x31; // LCD_display_manual.pdf page 18, hex 31 stand for 00110001, which is displayed as "1"
                xQueueSend( lcd_queue, &c, 10 ); // send that character to the queue to be displayed
                vTaskDelay(100*5 / portTICK_RATE_MS);
            }
            */
            break;
